...
export default function LanguagesPage() {
    ...
    const { data: languages, isLoading } = useQuery({
        queryKey: ["languages"],
        queryFn: async () => (await fetch("/api/admin/languages").then(res => res.json())),
    });
    ...
    const { register, handleSubmit, reset, formState: { errors } } = useForm<LanguageForm>({
        resolver: zodResolver(LanguageSchema),
    });

    const filtered = languages?.filter((c: any) =>
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className="flex items-center gap-2 bg-pink-600 hover:bg-pink-500 text-white px-4 py-2 rounded-lg"
                >
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-pink-500/50"
                />
            </div>
                    ...
                    <div className="bg-neutral-900 border border-white/10 rounded-2xl w-full max-w-lg p-6 space-y-6">
                        ...
                        <form onSubmit={handleSubmit((data) => createMutation.mutate(data))} className="space-y-4">
                            ...
                            <div className="pt-4 flex gap-3">
                                ...
                                <button type="submit" className="flex-1 px-4 py-2 rounded-lg bg-pink-600 hover:bg-pink-500 text-white">Create</button>
                            </div>
                        </form>
                    </div>
                </div>
