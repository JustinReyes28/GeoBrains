...
export default function LandmarksPage() {
    ...
    const { data: landmarks, isLoading } = useQuery({
        queryKey: ["landmarks"],
        queryFn: async () => (await fetch("/api/admin/landmarks").then(res => res.json())),
    });

    const createMutation = useMutation({
        mutationFn: async (data: LandmarkForm) => {
    ...
    const { register, control, handleSubmit, reset, formState: { errors } } = useForm<LandmarkForm>({
        resolver: zodResolver(LandmarkSchema),
    });

    const filtered = landmarks?.filter((l: any) =>
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h1 className="text-3xl font-bold text-white">Landmarks</h1>
                <button
                    onClick={() => setIsModalOpen(true)}
                    className="flex items-center gap-2 bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded-lg"
                >
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                />
            </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
                    <div className="bg-neutral-900 border border-white/10 rounded-2xl w-full max-w-lg p-6 space-y-6 my-8">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-bold text-white">Add Landmark</h2>
                        ...
                        <form onSubmit={handleSubmit((data) => createMutation.mutate(data))} className="space-y-4">
                            ...
                            <div className="pt-4 flex gap-3">
                                ...
                                <button type="submit" disabled={createMutation.isPending} className="flex-1 px-4 py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-white disabled:opacity-50">
