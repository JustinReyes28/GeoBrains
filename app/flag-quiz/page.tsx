...
import { ArrowRight, CheckCircle2, XCircle, Trophy, RotateCcw, Home, Flag, Check, X } from 'lucide-react';
...
export default function FlagQuizPage() {
    ...
    const fetchQuestions = async () => {
        try {
            setLoading(true);
            const res = await fetch('/api/flags/random?count=10');
    ...
    const handleNext = async () => {
        if (currentIndex < questions.length - 1) {
            ...
            setShowResult(true);
            try {
                await recordQuizScore(score, questions.length, 'flag-id');
            } catch (error) {
        ...
        return (
            <div className="min-h-screen bg-bg-primary text-text-primary flex items-center justify-center">
                <div className="text-center">
                    <Flag className="w-12 h-12 text-red-400 animate-bounce mx-auto mb-4" />
                    <h2 className="text-xl font-bold">Loading Flags...</h2>
                </div>
            </div>
        );
    }
    ...
    if (showResult) {
        return (
            <div className="min-h-screen bg-bg-primary text-text-primary flex items-center justify-center p-4">
                <div className="glass-card max-w-md w-full p-8 rounded-2xl text-center border-blue-500/20 shadow-2xl shadow-blue-500/10">
                    <div className="w-20 h-20 bg-blue-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
                        <Trophy className="w-10 h-10 text-blue-400" />
                    </div>
                    <h1 className="text-3xl font-bold mb-2">Quiz Complete!</h1>
                    <p className="text-text-secondary mb-8">You are a flag expert.</p>
                    ...
                    <div className="space-y-3">
                        <button
                            ...
                            className="w-full py-4 rounded-xl bg-blue-600 text-white font-bold shadow-lg shadow-blue-500/20 hover:scale-[1.02] transition-transform flex items-center justify-center gap-2"
                        >
        ...
        <div className="min-h-screen bg-bg-primary text-text-primary p-4 md:p-8">
            <header className="max-w-4xl mx-auto mb-8 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="bg-red-500/10 text-red-400 px-4 py-2 rounded-full text-sm font-bold border border-red-500/20">
                        Score: {score}
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-2xl mx-auto space-y-8">
                <div className="glass-card p-12 rounded-3xl border-red-500/20 relative overflow-hidden bg-gradient-to-br from-black/80 to-red-900/10 flex flex-col items-center text-center">
                    <div className="absolute top-0 right-0 p-4 opacity-10">
                        <Flag className="w-32 h-32" />
                    </div>

                    <h2 className="text-sm uppercase tracking-wider text-red-300 mb-8 font-semibold">Which country does this flag belong to?</h2>

                    <div className="mb-10 relative">
                        <div className="absolute -inset-4 bg-red-500/20 blur-xl rounded-full opacity-50"></div>
                        <img
                            src={`https://flagsapi.com/${currentQ.countryCode}/shiny/64.png`}
                            alt="Flag to guess"
                            className="w-32 h-32 object-contain drop-shadow-2xl relative z-10 scale-150"
                        />
                    </div>
                ...
                {isAnswered && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                        <div className={cn(
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-text-secondary text-sm">
                                    That is the flag of <span className="font-bold text-white">{currentQ.correctAnswer}</span>.
                                </p>
                            </div>
                        </div>
                        ...
                        <button
                            onClick={handleNext}
                            className="w-full py-4 rounded-xl bg-blue-600 text-white font-bold shadow-xl shadow-blue-500/20 hover:scale-[1.02] transition-transform active:scale-95 flex items-center justify-center gap-2"
                        >
